...
data ASTProgram =
  Program String ASTStatement -- Contains the program name (String) and the main statement (ASTStatement), which represents the program body. Example: PROGRAM MyProgram { x = 5; }
  deriving (Show) -- Enables automatic generation of `show` function for debugging and printing ASTs.
...
instance Applicative Parse2 where
  ...
  (Parse pf) <*> (Parse px) =
    Parse $ \s ->
      [(f x, s'') | (f, s') <- pf s, (x, s'') <- parse (Parse px) s'] -- Applies the function produced by 'pf' to the value produced by 'px'.
...
many1 p = do
  x <- p
  xs <- many p
  return (x : xs) -- Parses 'p' once, then parses 'p' zero or more times.
...
sat p = do
  ...
  if p c
    then return c
    else mzero -- Parses a character using 'item'. If the predicate 'p' is true for that character, it returns the character; otherwise, it fails.
...
string (c:cs) = do
  _ <- expectChar c
  _ <- string cs
  return (c : cs) -- Parses the first character, then recursively parses the rest of the string.
...
integer = do
  ...
  spaces -- Consumes any trailing spaces.
  if null ds
    then mzero -- If there are no digits, the parse fails.
    else return
           $ case sign of
...
float = do
  ...
  spaces -- Consumes any trailing spaces.
  if null intPart || null fracPart
    then mzero -- Requires both integer and fractional parts.
    else return
           $ case sign of
...
operator =
  expectChar '+' <|> expectChar '-' <|> expectChar '*' <|> expectChar '/' -- Parses one of the arithmetic operators.
...
expr :: Parse2 ASTExpr
expr = try parseNumber <|> try parseVariable <|> parseOpExpr -- Tries to parse an operator expression, a number, or a variable.
...
parseNumber :: Parse2 ASTExpr
parseNumber = try parseSignedFloat <|> parseSignedInt -- Tries to parse a float or an integer.

parseSignedInt :: Parse2 ASTExpr
parseSignedInt = do
  sign <- optional (expectChar '-')
  digits <- digit_seq
  spaces
  if null digits
    then mzero
...
parseSignedFloat :: Parse2 ASTExpr
parseSignedFloat = do
  sign <- optional (expectChar '-')
  intPart <- digit_seq
  _ <- expectChar '.'
  ...
  if null intPart || null fracPart
    then mzero
    else let f = read (intPart ++ "." ++ fracPart)
          in return
               $ FloatConst
                   (if sign == Just '-'
                      then -f
...
parseIfElse = do
  ...
  spaces -- Parses "IF".
  cond <- pred_expr
  spaces -- Parses the condition (a predicate expression).
  string "THEN"
  spaces -- Parses "THEN".
  s1 <- statement
  spaces -- Parses the 'then' branch statement.
  string "ELSE"
  spaces -- Parses "ELSE".
...
parseWhile = do
  ...
  spaces -- Parses "WHILE".
  cond <- pred_expr
  spaces -- Parses the loop condition.
  string "DO"
  spaces -- Parses "DO".
...
parseBlock = do
  expectChar '{'
  spaces -- Expects an opening curly brace.
  stmts <- statement_seq -- Parses a sequence of statements within the block.
  spaces
  expectChar '}' -- Expects a closing curly brace.
...
program = do
  string "PROGRAM"
  spaces -- Parses "PROGRAM".
  name <- program_name
  spaces -- Parses the program name.
...
main = do
  let code =
        "PROGRAM Demo { x = + 1 2; y = * x 3; IF == x y THEN NOOP ELSE x = + x 1 }."
